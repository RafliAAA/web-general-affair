...
const CreateProcurementModal = ({ open, onClose, onCreate, actualizationId, endUser }: Props) => {
  ...
  // Saat modal dibuka, set actualization_id dari props
  useEffect(() => {
    if (open) {
      setForm((prev) => ({
        ...prev,
        actualization_id: actualizationId || "",
        end_user: endUser || prev.end_user,
      }));
    }
  }, [open, actualizationId, endUser]);
